...
const SKILLS_SRC = path.join(__dirname, '..', 'packages');
...
if (args.includes('--help') || args.includes('-h')) {
  log(`
${bold}skills-for-agriculture${reset} — decision-making and guidance skills for farmers

${bold}Usage:${reset}
  npx @human-avatar/skills-for-agriculture [options]
...
${bold}After install:${reset}
  Restart Claude Code and type ${cyan}/s4ag${reset} to get started.
  Or jump straight to a category: ${cyan}/s4ag-soil${reset} ${cyan}/s4ag-pests${reset} ${cyan}/s4ag-seasons${reset}
`);
  process.exit(0);
}
...
function cleanLegacySettings(settingsPath, legacyDir) {
  ...
  try {
    ...
    settings.pluginDirectories = settings.pluginDirectories.filter(
      (d) => !path.resolve(d).includes('skills-for-agriculture')
    );
...
async function uninstall() {
  log();
  log(`${bold}Uninstalling skills-for-agriculture…${reset}`);
  log();
...
async function install() {
  log();
  log(`${bold}${cyan}skills-for-agriculture${reset}`);
  ...
  // Remove stale skills — previous versions used unprefixed names (e.g. "logic" → "s4ag-pests")
  const currentSet = new Set(names);
  if (fs.existsSync(skillsDir)) {
    for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      const unprefixed = entry.name.replace(/^s4ag-?/, '');
      const isOurs = currentSet.has(entry.name) || names.some((n) => n.replace(/^s4ag-?/, '') === unprefixed);
  ...
  log(`  ${cyan}/s4ag${reset}                ${dim}— route to the right skill${reset}`);
  log(`  ${cyan}/s4ag-soil${reset}           ${dim}— soil health, amendments, biology${reset}`);
  log(`  ${cyan}/s4ag-pests${reset}          ${dim}— IPM, biological controls, disease${reset}`);
  log(`  ${cyan}/s4ag-seasons${reset}        ${dim}— rotation, succession, year-round planning${reset}`);
  log(`  ${cyan}/s4ag-regenerative${reset}   ${dim}— transition to regenerative agriculture${reset}`);
  log();
}
